...
func main() {
	...
	testConfigurationManagement(service)
...
func testSimpleChat(service *llm.Service) {
	...
	for i, question := range testQuestions {
		...
		response, err := service.SimpleChat(ctx, question)
		if err != nil {
...
func testConfigurationManagement(service *llm.Service) {
	// Get original config
	originalConfig := service.GetConfig()
	fmt.Printf("   Original config: model=%s, temperature=%.2f, maxTokens=%d\n",
		originalConfig.Model, originalConfig.Temperature, originalConfig.MaxTokens)

	// Test model list
	models := service.GetAvailableModels()
	fmt.Printf("   ✓ Available models: %d\n", len(models))
	fmt.Printf("   ✓ Common models: ")
	commonModels := []string{"gpt-3.5-turbo", "gpt-4"}
	for i, model := range commonModels {
		if i > 0 {
			fmt.Print(", ")
		}
		fmt.Print(model)
	...
	fmt.Printf("   Updated config: model=%s, temperature=%.2f, maxTokens=%d\n",
		newConfig.Model, newConfig.Temperature, newConfig.MaxTokens)

	// Test system message update
	customSystemMsg := "你是一个专业的技术助手，专门回答编程相关问题。回复要简洁专业。"
	service.SetSystemMessage(customSystemMsg)
	fmt.Printf("   ✓ System message updated\n")

	// Restore original config
	service.UpdateConfig(originalConfig.Model, originalConfig.Temperature, originalConfig.MaxTokens)
	service.SetSystemMessage(originalConfig.SystemMessage)
